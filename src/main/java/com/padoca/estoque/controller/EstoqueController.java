...
public class EstoqueController {
    ...
    public String cadastrarMovimentacao(@ModelAttribute Movimentacao movimentacao, 
        ...
        //busca o produto pelo id que veio informado ao select do html, para depois atualizar a quantidade do produto
        Produto produto = produtoRepository.findById(movimentacao.getProduto().getId()).orElseThrow();

        //Vai atualizar a quantidade de produto, somando ou subtraindo a quantidade da movimentação, dependendo do tipo da movimentação
        if (movimentacao.getTipo().equals("ENTRADA"))
            produto.setQuantidade( produto.getQuantidade() + movimentacao.getQuantidade());
        else
            produto.setQuantidade( produto.getQuantidade() - movimentacao.getQuantidade());

        //primeiro deve salvar a movimentação depois o produto, para garantir que a movimentação tenha um id gerado e possa ser associada ao produto
